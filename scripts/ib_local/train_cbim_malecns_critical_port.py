"""Train the passive critical-port MaleCNS CBIM on continuous OWT."""
import os
import sys
...
def main():
    ...
    parser.add_argument("--output", type=Path,
                        default=Path("results/cbim_malecns_critical_port_3000"))
    ...
    source_files = ["scripts/ib_local/cbim_malecns_critical_port.py",
                    "scripts/ib_local/cbim_malecns_v3.py",
    ...
    config = {
        ...
        "state_channels": 2 * model.d + 3, "semantic_state_channels": model.d,
        "nodes": model.L,
        "queries": 4, "heads": 4,
        "probe_epsilon": model.probe_epsilon,
        "controller_ema": model.controller_ema,
        "controller_lr": model.controller_lr,
        "flux_ema": model.flux_ema,
        "flux_lr": model.flux_lr,
        "max_critical_a": model.max_critical_a,
        "critical_measure_interval": model.checkpoint_tokens,
    ...
    try:
        ...
        while step < args.steps:
            ...
            if step == 1 or step % 10 == 0 or step == args.steps:
                field, delta, critical_a, lambda_ema, flux_ema = model.unpack(state.detach())
                ...
                row = {
                    ...
                    "port_balance_residual": float(diagnostics["port_balance_residual"]),
                    "incident_energy": float(diagnostics["incident_energy"]),
                    "outgoing_energy": float(diagnostics["outgoing_energy"]),
                    "accommodation_mean": float(diagnostics["accommodation_mean"]),
                    "accommodation_max": float(diagnostics["accommodation_max"]),
                    "critical_a_mean": float(diagnostics["critical_a_mean"]),
                    "critical_a_max": float(diagnostics["critical_a_max"]),
                    "lambda_mean": float(diagnostics["lambda_mean"]),
                    "lambda_max": float(diagnostics["lambda_max"]),
                    "flux_ema_mean": float(diagnostics["flux_ema_mean"]),
                    "flux_ema_max": float(diagnostics["flux_ema_max"]),
                ...
                atomic_json(args.output / "live_state.json", {
                    **row,
                    ...
                    "node_critical_a": critical_a[0].cpu().tolist(),
                    "node_lambda": lambda_ema[0].cpu().tolist(),
                    "node_flux": flux_ema[0].cpu().tolist(),
                    "node_probe": delta[0].square().mean(-1).sqrt().cpu().tolist(),
                })
