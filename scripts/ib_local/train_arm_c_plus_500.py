"""Controlled 500-Step Benchmark: Arm C vs Arm C+ (Answer Self-Conditioning).
Compares:
1. Arm_C_Problem_Only: Learned Isoenergetic Corrective Flow with P_problem only
2. Arm_C_Plus_Answer: Learned Isoenergetic Corrective Flow with P_problem + P_ans^(k-1)

Evaluates on all 1,000 test puzzles across K in [1, 2, 4, 8, 16, 32, 64, 128, 256].
Strictly identical seed, data order, batch size, and multi-horizon training protocol.
"""
import os
import sys
...
from scripts.ib_local.cbim_sudoku_corrective_answer import ArmCPlusCBIMSudokuModel
...
def run_benchmark():
    print("=" * 105)
    print("   HEAD-TO-HEAD 500-STEP BENCHMARK: ARM C (PROBLEM-ONLY) vs ARM C+ (ANSWER-CONDITIONED)")
    print("=" * 105)
    ...
    configs = [
        ("Arm_C_Problem_Only", False),
        ("Arm_C_Plus_Answer_Conditioned", True)
    ]

    all_results = {}
    checkpoint_dir = Path("checkpoints/arm_c_plus_500")
    checkpoint_dir.mkdir(parents=True, exist_ok=True)

    for name, ans_cond in configs:
        print("\n" + "=" * 90)
        print(f"   TRAINING 500 STEPS: {name} (Answer Conditioned: {ans_cond})")
        print("=" * 90)

        inner = ArmCPlusCBIMSudokuModel(
            vocab_size=11, d_channels=128, ponder_steps=16,
            answer_conditioned=ans_cond, alpha_max=0.25
        ).to(device)
        ...
        for step in range(1, 501):
            ...
            if step % 100 == 0 or step == 1:
                ...
                print(f"[{name:30s}] Step {step:3d}/500 | K: {k_step:2d} | Loss: {loss.item():6.3f} | VRAM: {vram_mb:6.1f}MB", flush=True)
        ...
        torch.save({"model": model.state_dict(), "name": name, "answer_conditioned": ans_cond, "params": params}, ckpt_path)
    ...
    print("\n" + "=" * 115)
    print("   HEAD-TO-HEAD COMPARISON: ARM C (PROBLEM-ONLY) vs ARM C+ (ANSWER-CONDITIONED)")
    print("=" * 115)
    print(f"{'K':>4s} | {'Arm C Acc':>12s} | {'Arm C+ Acc':>12s} | {'Delta Acc':>11s} | {'Arm C Loss':>12s} | {'Arm C+ Loss':>12s} | {'Delta Loss':>12s}")
    print("-" * 115)
    for k in [1, 2, 4, 8, 16, 32, 64, 128, 256]:
        c = all_results["Arm_C_Problem_Only"][k]
        cp = all_results["Arm_C_Plus_Answer_Conditioned"][k]
        d_acc = (cp["cell_accuracy"] - c["cell_accuracy"]) * 100
        d_loss = cp["mean_loss"] - c["mean_loss"]
        print(f"{k:4d} | {c['cell_accuracy']*100:11.2f}% | {cp['cell_accuracy']*100:11.2f}% | {d_acc:+10.2f}% | {c['mean_loss']:12.3f} | {cp['mean_loss']:12.3f} | {d_loss:+12.3f}")

    out_file = Path("results/ablation_arm_c_plus_500.json")
