"""Train open-boundary MaleCNS kinetic CBIM v3 on continuous OWT."""
import os
import sys
...
def main():
    ...
    parser.add_argument("--output", type=Path,
                        default=Path("results/cbim_malecns_v3_3000"))
    ...
    source_files = ["scripts/ib_local/cbim_malecns_v3.py",
    ...
    config = {
        ...
        "state_channels": model.d + 2, "nodes": model.L,
        "queries": 4, "heads": 4,
    ...
    try:
        ...
        while step < args.steps:
            ...
            if step == 1 or step % 10 == 0 or step == args.steps:
                field, resource, fatigue = model.unpack(state.detach())
                ...
                row = {
                    ...
                    "boundary_input_work": float(
                        diagnostics["boundary_input_work"]),
                    "boundary_output_work": float(
                        diagnostics["boundary_output_work"]),
                    "energy_balance_residual": float(
                        diagnostics["energy_balance_residual"]),
                    "write_rate_mean": float(diagnostics["write_rate_mean"]),
                    "outflow_rate_mean": float(
                        diagnostics["outflow_rate_mean"]),
                    "resource_mean": float(diagnostics["resource_mean"]),
                    "resource_min": float(diagnostics["resource_min"]),
                    "fatigue_mean": float(diagnostics["fatigue_mean"]),
                    "fatigue_max": float(diagnostics["fatigue_max"]),
                ...
                atomic_json(args.output / "live_state.json", {
                    **row,
                    ...
                    "node_resource": resource[0].cpu().tolist(),
                    "node_fatigue": fatigue[0].cpu().tolist(),
                })
