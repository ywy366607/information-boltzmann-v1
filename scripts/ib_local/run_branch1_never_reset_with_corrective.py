"""Branch 1: Never-Reset + Port Injection + Corrective Flow (Arm C).
...
@torch.no_grad()
def evaluate_branch1(model, test_in: np.ndarray, test_lbl: np.ndarray,
...
def main():
    ...
    out_dir = Path("results/ablation_branch1_never_reset_with_corrective")
    out_dir.mkdir(parents=True, exist_ok=True)

    print("=" * 95)
    print("   BRANCH 1: NEVER RESET + PORT INJECTION + CORRECTIVE FLOW (d=256, 500 STEPS)")
    print("=" * 95)
    ...
    for step in range(1, steps + 1):
        ...
        if step % 50 == 0 or step == 1:
            print(f"[Branch 1] Step {step:3d}/{steps} | K: {k_step:2d} | Loss: {accum_loss:6.3f} | LR: {current_lr:.1e}", flush=True)

    print(f"\n[Branch 1] Training complete in {(time.perf_counter()-start_time)/60:.2f} min! Running Never-Reset Evaluation...")
    val_results = evaluate_branch1(model, test_in, test_lbl, persistent_state, horizons=[1, 2, 4, 8, 16, 32, 64, 128, 256], batch_size=32)
    print("\n" + "=" * 80)
    print("   BRANCH 1 (NEVER RESET + INJECTION + CORRECTIVE FLOW) FINAL RESULTS")
    print("=" * 80)
