"""Comprehensive Deep Animation Suite for CBIM Reasoning Dynamics:
Extends pondering depth to full K=512 and deep multi-cycle wave/fractal horizons:
1. present/cbim_pondering_crystallization_k512_easy.gif (K=1..512, 67.9% Acc)
2. present/cbim_pondering_crystallization_k512_hard.gif (K=1..512, 38.3% Acc)
3. present/cbim_torus_wave_flow_deep.gif (K=1..64, full 360-deg rotation, multi-circuit torus advection)
4. present/cbim_fractal_basin_time_evolution_deep.gif (K=1..64, deep multi-generation fractal folding & vortex rollup)
"""
import os
import sys
...
@torch.no_grad()
def generate_k512_animation(model, inp, lbl, name: str, out_path: Path, max_k: int = 512):
    print(f"\n[1/4] Generating Full K=1..{max_k} Crystallization Animation for {name}...", flush=True)
    ...
    # Sample 50 frames across K=1..512
    sampled_steps = sorted(list(set(
        list(range(1, 17)) +
        [20, 24, 28, 32, 40, 48, 56, 64, 80, 96, 112, 128, 144, 160, 192, 224, 256, 288, 320, 352, 384, 416, 448, 480, 512]
    )))
    ...
    for k in range(1, max_k + 1):
        ...
        if k in sampled_steps:
            ...
            fig.suptitle(f"CBIM Latent Pondering Dynamics ({name}) | Time Axis K = {k:03d} / {max_k}", fontsize=16, color="#e0e8ff", y=0.96, fontweight="bold")
    ...
    print(f"Saved full K={max_k} animation to {out_path}", flush=True)


@torch.no_grad()
def generate_deep_torus_wave_animation(model, inp, out_dir: Path, max_k: int = 64):
    print(f"\n[3/4] Generating Deep 3D Donut Torus Multi-Cycle Wave Flow (K=1..{max_k})...", flush=True)
    ...
    for k in range(1, max_k + 1):
        ...
        # 3D Matplotlib Render with full 360-degree rotation
        fig = plt.figure(figsize=(8, 7), facecolor="#060614")
        ax = fig.add_subplot(111, projection="3d", facecolor="#060614")
        fig.suptitle(f"3D Periodic Torus Multi-Cycle Wave Streaming: K = {k:02d} / {max_k}", fontsize=15, color="#e0e8ff", y=0.95, fontweight="bold")

        azim_angle = (k * 360.0 / max_k) % 360
        ax.view_init(elev=32, azim=azim_angle)
    ...
    gif_path = out_dir / "cbim_torus_wave_flow_deep.gif"
    frames[0].save(
        str(gif_path),
        save_all=True,
        append_images=frames[1:],
    ...
    print(f"Saved deep 3D Torus wave animation to {gif_path}", flush=True)


@torch.no_grad()
def generate_deep_fractal_basin_animation(model, inp, out_dir: Path, max_k: int = 64, res: int = 90, radius: float = 2.5):
    print(f"\n[4/4] Generating Deep Multi-Generation Fractal Basin Folding (K=1..{max_k})...", flush=True)
    clue_field = model.model.clue_proj(model.model.embed_tokens(inp).view(1, 9, 9, model.model.d))
    ...
    print(f"Saved deep fractal basin evolution animation to {gif_path}", flush=True)
...
def main():
    ...
    print("=" * 95)
    print("   DEEP MULTI-CYCLE ANIMATION SUITE: K=512 CRYSTALLIZATION & DEEP K=64 DYNAMICS")
    print("=" * 95)
    ...
    # 1. Easy Puzzle (35 clues, reaches 67.9% Acc at K=512)
    ...
    generate_k512_animation(model, inp_easy, lbl_easy, "Easy 35-Clue (67.9% Acc)", out_dir / "cbim_pondering_crystallization_k512_easy.gif", max_k=512)

    # 2. Hard Extreme Puzzle (17 clues, 38.3% Acc at K=512)
    ...
    generate_k512_animation(model, inp_hard, lbl_hard, "Hard 17-Clue Extreme (38.3% Acc)", out_dir / "cbim_pondering_crystallization_k512_hard.gif", max_k=512)

    # 3. Deep 3D Donut Torus Multi-Cycle Circulation (K=1..64)
    generate_deep_torus_wave_animation(model, inp_hard, out_dir, max_k=64)

    # 4. Deep Fractal Basin Multi-Generation Folding (K=1..64)
    generate_deep_fractal_basin_animation(model, inp_hard, out_dir, max_k=64, res=80, radius=2.5)

    print("\n" + "=" * 80)
    print("   ALL DEEP ANIMATIONS COMPLETED AND SAVED TO present/")
    print("=" * 80)
