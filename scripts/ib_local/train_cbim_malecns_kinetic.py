"""Train the role-separated MaleCNS kinetic CBIM on continuous OWT."""
import os
import sys
...
from scripts.ib_local.cbim_malecns_kinetic import CBIMMaleCNSKinetic
...
def main():
    ...
    parser.add_argument("--output", type=Path,
                        default=Path("results/cbim_malecns_kinetic_3000"))
    ...
    source_files = ["scripts/ib_local/cbim_malecns_kinetic.py",
                    "scripts/ib_local/cbim_cuda_graph.py", __file__]
    model = CBIMMaleCNSKinetic(
        args.graph, velocities=args.velocities, content_dim=args.content_dim).cuda()
        ...
        while step < args.steps:
            ...
            if step == 1 or step % 10 == 0 or step == args.steps:
                ...
                row = {
                    ...
                    "injection_norm": float(diagnostics["injection_norm"]),
                    "source_energy_delta": float(
                        diagnostics["source_energy_delta"]),
                    "dissipated_energy": float(
                        diagnostics["dissipated_energy"]),
                    "energy_balance_residual": float(
                        diagnostics["energy_balance_residual"]),
                    "alpha_mean": float(diagnostics["alpha_mean"]),
                    "gamma_mean": float(diagnostics["gamma_mean"]),
                    "gamma_spatial_std": float(diagnostics["gamma_spatial_std"]),
                    "gamma_channel_std": float(
                        diagnostics["gamma_channel_std"]),
                    "gamma_min": float(diagnostics["gamma_min"]),
                    "gamma_max": float(diagnostics["gamma_max"]),
                    "learned_gamma_mean": float(
                        diagnostics["learned_gamma_mean"]),
                    "feedback_gamma_mean": float(
                        diagnostics["feedback_gamma_mean"]),
                    "feedback_active_rate": float(
                        diagnostics["feedback_active_rate"]),
                    "safety_projection_rate": 0.0,
                    "collision_angle_abs_mean": float(
                        diagnostics["collision_angle_abs_mean"]),
                    "grad_norm": float(runner.grad_norm),
                    "spatial_variance": float(
                        field.var(1, unbiased=False).mean()),
                ...
                atomic_json(args.output / "live_state.json", {
                    **row,
                    ...
                    "node_gamma": diagnostics["gamma_node_mean"].detach().cpu().tolist()})
