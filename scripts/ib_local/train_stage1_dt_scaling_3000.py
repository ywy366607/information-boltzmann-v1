"""Stage 1 of Systematic Alignment: Time-Step Continuum Scaling (dt = 1.0 / K).
Core Hypothesis:
In the discrete model, each microstep applies a fixed rotation angle alpha_k.
When K extrapolates from 2 to 64, accumulating 64 large rotations causes overshoot!
With time-step continuum scaling:
  dt = 1.0 / K
  collision_angle = dt * theta_scale * angle_mlp
  corrective_angle = dt * alpha_max * sigmoid(mean_gate)
The total integrated rotation across K steps is scale-invariant:
  sum_{k=1}^K alpha_k ~ K * (1/K) * alpha = alpha (Constant!)
This refines the numerical ODE integration with finer mesh (O(dt^2) error decay)
instead of overshooting the energy basin!

Configuration:
- Architecture: Master Unified CBIM with Never-Reset, H-boundary 2-Port scattering theta(F,P), and H-exit bath.
- New mechanism: Time-Step Continuum Scaling (dt = 1.0 / K) on collision and corrective flow.
...
class ContinuousConditionedLieCollision(nn.Module):
    """Lie Collision with Continuous Time-Step Scaling: angles = dt * theta_scale * MLP."""
    ...
    def forward(self, f_tr: torch.Tensor, cond: torch.Tensor, dt: float = 1.0) -> torch.Tensor:
        b, h, w, d = f_tr.shape
        cat = torch.cat([f_tr, cond], dim=-1)
        # Continuous time scaling: rotation angle proportional to dt!
        angles = (dt * self.theta_scale) * self.angle_mlp(cat)  # [B, 9, 9, dim_active//2]
...
class Stage1ContinuousTimeCBIMSudokuModel(nn.Module):
    """CBIM Sudoku Model with Stage 1 Time-Step Continuum Scaling (dt = 1/K)."""
    ...
    def __init__(self, vocab_size: int = 11, d_channels: int = 256, alpha_max: float = 0.25):
        ...
        # 3. Continuous Lie Collision (with dt scaling)
        self.collision = ContinuousConditionedLieCollision(d=d_channels)

        # 4. Continuous Arm C Corrective Flow (with dt scaling)
    ...
    def forward_stream_step(self, persistent_state: torch.Tensor, inp: torch.Tensor, k_step: int) -> Tuple[torch.Tensor, torch.Tensor, Dict[str, Any]]:
        ...
        # 2. Microstep Loop K: Transport -> Continuous Collision(dt) -> Continuous Corrective Flow(dt)
        for _ in range(k_step):
            ...
            # Continuous rotation angle alpha_k scaled by dt!
...
def main():
    parser = argparse.ArgumentParser(description="Stage 1 Alignment: Continuous Time-Step Scaling (dt = 1/K)")
    ...
    parser.add_argument("--output-dir", type=str, default="results/stage1_dt_scaling_3000", help="Output directory")
    ...
    print("=" * 110)
    print("   STAGE 1 ALIGNMENT: TIME-STEP CONTINUUM SCALING (dt = 1.0 / K)")
    print(f"   Architecture: Master Unified CBIM + dt-Scaled Continuous Collision & Corrective Flow")
    print(f"   Training Horizons: STRICTLY K in [1, 2] | Steps: {args.steps} | Eff Batch: {args.micro_batch_size * args.grad_accum_steps}")
    print(f"   Goal: Test whether dt = 1/K eliminates deep pondering overshoot (K=4..512) when trained on ONLY K=1,2!")
    print("=" * 110)
    ...
    model = Stage1ContinuousTimeCBIMSudokuModel(vocab_size=11, d_channels=args.d_channels).to(device)
    ...
    for step in range(1, args.steps + 1):
        ...
        if step % args.eval_interval == 0 or step == args.steps:
            ...
            print(f"\n[INTERIM dt=1/K EXTRAPOLATION Step {step}] Trained ONLY on K in [1, 2]:", flush=True)
            ...
            if acc_k16 > best_cell_acc:
                ...
                }, out_path / "Best_Stage1_ContinuousTime_d256.pt")
                print(f"Saved new best model to {out_path / 'Best_Stage1_ContinuousTime_d256.pt'}\n", flush=True)
    ...
    print("   (Model trained ONLY on K=1, 2! With dt = 1.0 / K Continuous Time-Step Scaling!)")
    print("=" * 105)
    ...
    with open(out_path / "final_report.json", "w", encoding="utf-8") as f:
        json.dump({
            "mechanism": "Time-Step Continuum Scaling (dt = 1.0 / K)",
