"""Branch 3: 100% Language-Aligned Pure Physical Flow (Never-Reset + NO Additive Injection + Pure TC).
500-step controlled ablation on Sudoku-Extreme (d=256, Cosine LR).
Core mechanics:
- State persistence: Never Reset (persistent_state carried over across batches).
- Input ingestion: NO ADDITIVE INJECTION! curr = persistent_state.
  (Eliminates the clue-ghost superposition conflict between unrelated puzzles).
- Microsteps: Pure TC (Unitary Cayley Transport + Problem-Conditioned Lie algebra Collision).
- Multi-scale loss supervision: K in [2, 4, 8, 16, 32, 64].
"""
import os
import sys
...
@torch.no_grad()
def evaluate_branch3(model, test_in: np.ndarray, test_lbl: np.ndarray,
    ...
    for k in horizons:
        ...
        for s in range(0, len(test_in), batch_size):
            ...
            # NO ADDITIVE INJECTION: State flows smoothly as in language!
            curr = state[:b].clone()
...
def main():
    ...
    out_dir = Path("results/ablation_branch3_never_reset_no_addition_pure_tc")
    out_dir.mkdir(parents=True, exist_ok=True)

    print("=" * 105)
    print("   BRANCH 3: NEVER RESET + NO ADDITIVE INJECTION + PURE TC (100% LANGUAGE-ALIGNED, 500 STEPS)")
    print("=" * 105)
    ...
    for step in range(1, steps + 1):
        ...
        for _ in range(accum):
            ...
            with torch.amp.autocast("cuda", dtype=torch.float16):
                ...
                # NO ADDITIVE INJECTION: Direct continuous flow from persistent state!
                curr = persistent_state.clone()
        ...
        if step % 50 == 0 or step == 1:
            print(f"[Branch 3] Step {step:3d}/{steps} | K: {k_step:2d} | Loss: {accum_loss:6.3f} | LR: {current_lr:.1e}", flush=True)

    print(f"\n[Branch 3] Training complete in {(time.perf_counter()-start_time)/60:.2f} min! Running Never-Reset Evaluation...")
    val_results = evaluate_branch3(model, test_in, test_lbl, persistent_state, horizons=[1, 2, 4, 8, 16, 32, 64, 128, 256], batch_size=32)
    print("\n" + "=" * 80)
    print("   BRANCH 3 (NEVER RESET + NO ADDITION + PURE TC) FINAL RESULTS")
    print("=" * 80)
