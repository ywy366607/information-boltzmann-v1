"""Train critical Maxwell-boundary MaleCNS CBIM on continuous OWT."""
import os
import sys
...
def main():
    ...
    parser.add_argument("--output", type=Path,
                        default=Path("results/cbim_malecns_critical_3000"))
    ...
    source_files = ["scripts/ib_local/cbim_malecns_critical.py",
    ...
    config = {
        ...
        "state_channels": 2 * model.d + 2, "semantic_state_channels": model.d,
        "nodes": model.L,
        "queries": 4, "heads": 4,
    ...
    try:
        ...
        while step < args.steps:
            ...
            if step == 1 or step % 10 == 0 or step == args.steps:
                field, delta, conductance, lambda_ema = model.unpack(state.detach())
                ...
                row = {
                    ...
                    "conductance_mean": float(diagnostics["conductance_mean"]),
                    "conductance_max": float(diagnostics["conductance_max"]),
                    "lambda_mean": float(diagnostics["lambda_mean"]),
                    "lambda_max": float(diagnostics["lambda_max"]),
                    "global_lambda": float(diagnostics["global_lambda"]),
                ...
                atomic_json(args.output / "live_state.json", {
                    **row,
                    ...
                    "node_conductance": conductance[0].cpu().tolist(),
