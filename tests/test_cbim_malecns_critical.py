...
from scripts.ib_local.cbim_malecns_critical import (
    CBIMMaleCNSCritical,
    CriticalMaxwellBoundary,
)
...
def test_maxwell_boundary_is_bounded_and_differentiable():
    ...
    graph = graph_fixture()
    boundary = CriticalMaxwellBoundary(graph, vocab_size=32).double()
    field = torch.randn(2, 8, 64, dtype=torch.float64) * .1
    conductance = torch.zeros(2, 8, dtype=torch.float64)
    output, diagnostics = boundary(field, torch.tensor([1, 2]), conductance)
    assert output.shape == field.shape
    assert 0 <= diagnostics["accommodation_mean"] <= boundary.max_rate
    assert diagnostics["accommodation_max"] <= boundary.max_rate
    output.square().mean().backward()
    assert boundary.rate.weight.grad is not None
    assert torch.isfinite(boundary.rate.weight.grad).all()
...
def test_complete_critical_step_has_clean_roles_and_finite_controller(tmp_path):
    ...
    model = CBIMMaleCNSCritical(path, vocab_size=32).double()
    state = model.initial_state(2)
    for token in range(6):
        state, diagnostics = model.evolve(state, torch.tensor([token, token + 1]))
    field, delta, conductance, lambda_ema = model.unpack(state)
    assert state.shape == (2, 8, 130)
...
def test_cross_entropy_reaches_each_semantic_operator(tmp_path):
    ...
    loss.backward()
    parameters = (
        model.boundary.proposal[-1].weight,
        model.boundary.rate.weight,
